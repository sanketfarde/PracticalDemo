...
@Entity
@Table(name = "tbl_distribution_magazine")
public class DistributionMagzine {
...
/*
	...
	*/
	...
	/*@ElementCollection
	@CollectionTable(name = "butt_no", joinColumns = @JoinColumn(name = "id"))*/
	@Column(name = "butt_no", columnDefinition = "TEXT")
	private String buttNo = "";

	/*@ElementCollection
	@CollectionTable(name = "manufacturing_no", joinColumns = @JoinColumn(name = "id"))*/
	@Column(name = "manufacturing_no", columnDefinition = "TEXT")
	private String manufacturingNo = "";
...
/*	@Transient
	...
	*/
	...
	@Column(name = "return_weapon")
	private String returnWeapon;

	@Column(name = "balance_weapon")
...
/*
	...
	// This is for Butt Number
	@JsonIgnore
	public Optional<List<String>> getButtNoAsList() {
		if (buttNo == null || buttNo.trim().isEmpty()) {
			return Optional.empty();
		}
		try {
			return Optional.of(objectMappers.readValue(buttNo, new TypeReference<List<String>>() {}));
		} catch (JsonProcessingException e) {
			return Optional.empty();
		}
	}
	...
	public void setButtNoAsList(List<String> buttNoList) {
		try {
			this.buttNo = objectMappers.writeValueAsString(buttNoList);
		} catch (JsonProcessingException e) {
			this.buttNo = "";
		}
	}
	...
	@Transient
	public List<String> getButtNoList() {
		return getButtNoAsList().orElse(Collections.emptyList());
	}

	@JsonIgnore
	public Optional<List<String>> getManufacturingNoAsList() {
		if (manufacturingNo == null || manufacturingNo.trim().isEmpty()) {
			return Optional.empty();
		}
		try {
			return Optional.of(objectMapper.readValue(manufacturingNo, new TypeReference<List<String>>() {}));
		} catch (JsonProcessingException e) {
			return Optional.empty();
		}
	}
	...
	public void setManufacturingNoAsList(List<String> manufacturingNoList) {
		try {
			this.manufacturingNo = objectMapper.writeValueAsString(manufacturingNoList);
		} catch (JsonProcessingException e) {
			this.manufacturingNo = "";
		}
	}

	@Transient
	public List<String> getManufacturingNoList() {
		return getManufacturingNoAsList().orElse(Collections.emptyList());
	}
